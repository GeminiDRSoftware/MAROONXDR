...
``{'createSyntheticDark:individual': True}`` to ``uparms`` to get one
product per input frame instead. The synthetic darks are stored under the
``processed_dark`` caltype with the ``DARK_SYNTH`` tag.
...
``createSyntheticDarkFromCoeffs``, as a single value or as a list:
