    ...
    for name, item in primclass.__dict__.items():
        ...
        if isinstance(item, types.FunctionType):
            ...
            try:
                ...
                # Some primitives have no associated pex.config class (eg.
                # simple wrappers).  Skip the parameter fragment for those.
                print('WARNING: no parameters for ', primclass.__name__, name)
                continue
    ...
    for name, clss in inspect.getmembers(primmod, inspect.isclass):
        if clss.__module__ == primmod.__name__ and hasattr(clss, 'tagset'):
            ...
            try:
                ...
                # The parameter config class has no matching primitive method
                # in the primitive class.  Nothing to document, skip it.
                print('WARNING: no primitive for parameter class ',
                      clss.__name__, associated_primname)
                continue
