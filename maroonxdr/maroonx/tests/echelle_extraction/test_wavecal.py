...
def test_staticWavelengthSolution(caplog, filename):
    ...
    for fiber in [1, 2, 3, 4, 5]:
        assert hasattr(out_ad[0], f'WLS_STATIC_FIBER_{fiber}'), f"Static solution extension for fiber {fiber} not found"
        ...
        if fiber in requested_fibers:
            ...
            assert wls.shape[0] == n_orders, f"Static solution for fiber {fiber} has incorrect number of orders"
        else:
...
def test_fitAndApplyEtalonWls(caplog, filename):
    ...
    for fiber in [1, 2, 3, 4, 5]:
        assert hasattr(out_ad[0], f'WLS_DYNAMIC_FIBER_{fiber}'), f"Dynamic solution extension for fiber {fiber} not found"

        wls = getattr(out_ad[0], f'WLS_DYNAMIC_FIBER_{fiber}')

        if fiber in requested_fibers:
            n_orders = len(getattr(out_ad[0], f'REDUCED_ORDERS_FIBER_{fiber}'))
            assert wls.shape[0] == n_orders, f"Dynamic solution for fiber {fiber} has incorrect number of orders"
        else:
            assert wls.shape == (1, 1), f"Unrequested dynamic solution for fiber {fiber} found"
    ...
    assert expected_cols.issubset(peaks_cols), f"PEAKS table should contain columns: {expected_cols}"
